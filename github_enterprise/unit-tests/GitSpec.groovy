...
public class GitSpec extends JenkinsPipelineSpecification {
  ...
  def setup() {
    Git = loadPipelineScriptForTest("github_enterprise/git.groovy")
  ...
  def "Each Action in the args map is called as expected" () {
    ...
    then:
      ...
      y * getPipelineMock("sh")({if (it instanceof Map) it.script =~ /^git commit -m \"commit from Jenkins\"$/; else false})
    ...
    where:
      ...
      [commit: null]                           | 0 | 1 | 0
      [push: null]                             | 0 | 0 | 1
      [add: "files", commit: null]             | 1 | 1 | 0
      [add: "files", push: null]               | 1 | 0 | 1
      [commit: null, push: null]               | 0 | 1 | 1
      [add: "files", commit: null, push: null] | 1 | 1 | 1
  }
